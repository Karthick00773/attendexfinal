      ...
      <div style={{
        ...
        <div style={{
          ...
          <svg width="32" height="32" viewBox="0 0 24 24" fill="none"
            stroke="var(--accent, #6c47ff)" strokeWidth="2"
...
function useScreenshotCapture(userRole) {
  ...
  const saveToDB = async (url) => {
    await fetch(`${BACKEND_URL}/screenshots`, {
      ...
      headers: {
        ...
        Authorization: `Bearer ${localStorage.getItem('attendx_token')}`,
      },
      body: JSON.stringify({ url, taken_at: new Date().toISOString() }),
    });
  };

  const captureFrame = useCallback((stream) => {
  ...
  const doCapture = useCallback(async () => {
    ...
    try {
      ...
      formData.append('fileName', `screenshot_${Date.now()}.jpg`);
  ...
  const startCapturing = useCallback(async () => {
    try {
      ...
      console.error('[Screenshot] getDisplayMedia error:', err);
      localStorage.setItem(STORAGE_KEY, 'idle');
    }
  }, [doCapture]);
  ...
  return { showPopup, isCapturing, handleAllow, handleDeny };
}
