...
export default function HomePage() {
  ...
  // dashboard/me → { user, month_summary: { normal_hours, overtime_hours, total_hours, present_days, absent_days, leave_days }, today, pending_leaves, unread_notifs }
  // dashboard/overview → { today: { total_employees, present, absent }, pending_leaves, month_totals, month }
  const summary = dashboardStats?.month_summary || {};
  const user    = dashboardStats?.user || currentUser;

  // Leave balance: backend user has paid_leaves_total & paid_leaves_used
  ...
  return (
    <div className="page animate-fadeup">
      ...
      <div className="home-grid">
        ...
        {/* Admin/CEO today summary from real overview response */}
        ...
        {/* Notifications — real backend field is `text` not `body` */}
            ...
            {notifList.slice(0, 5).map(n => (
              <div key={n.id} className={`notif-item ${!n.is_read ? 'notif-unread' : ''}`}>
                ...
                <div className="notif-body">
                  {/* Real backend uses `text` field, not `body` */}
